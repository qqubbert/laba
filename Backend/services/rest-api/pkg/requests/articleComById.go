...
func GetCommentsByArticleId(db *sql.DB, articleID int) ([]ArticleComment, error) {
	query := `
		SELECT ac.id, ac.article_id, ac.author_id, ac.comm, CONCAT(u.FirstName, ' ', u.LastName) AS author
	...
	for rows.Next() {
		...
		if err := rows.Scan(&comment.Id, &comment.ArticleId, &comment.AuthorId, &comment.Comm, &comment.Author); err != nil {
			return nil, err
		}
